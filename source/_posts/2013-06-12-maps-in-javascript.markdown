...
So here's my better method [as a gist][map-gist] and [as a fiddle][map-fiddle] with a little extra code to show you how to use it; although the JSDoc comments should be enough. It may seem complicated in comparison to the method above, but it's a lot more powerful and isn't actually that intricate.
...
Because I'm using two arrays and not a single array, containing objects with a key and value property, I can search the keys array with `indexOf`, or potentially the [binary search function][binary-search] I talked about in my last post.
...
If you do decide to use a binary search, you should probably use a [set][cpp-set-docs] implementation in place of the keys array; that will keep your items in order for you. You should probably only bother with this if you can prove that searching the keys array is definitely a bottleneck. It should only be the case in very extreme circumstances.

[cpp-map-docs]: http://www.cplusplus.com/reference/map/map/
[cpp-set-docs]: http://www.cplusplus.com/reference/set/set/
[hash-map]: http://en.wikipedia.org/wiki/Hash_table
[simple-map-fiddle]: http://jsfiddle.net/Wolfy87/ATUSS/
[map-gist]: https://gist.github.com/Wolfy87/5759960
[map-fiddle]: http://jsfiddle.net/Wolfy87/Wuqag/
[binary-search]: /2013/06/08/searching-javascript-arrays-with-a-binary-search/
